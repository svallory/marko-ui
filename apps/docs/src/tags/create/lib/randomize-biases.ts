// Ported verbatim from shadcn/ui app/(app)/(create)/lib/randomize-biases.ts.
// Only the import paths changed (registry config + fonts live in this lib
// directory instead of "@/registry/config" / "./fonts" aliases).
import type {
  BaseColor,
  BaseColorName,
  Radius,
...
export const RANDOMIZE_BIASES: RandomizeBiases = {
  baseColors: (baseColors) => {
    // Cast: this repo's BASE_COLORS name union does not include "gray"
    // (shadcn's does), so the verbatim comparison needs widening. The
    // runtime behavior is identical either way.
    return baseColors.filter((c) => (c.name as string) !== "gray")
  },
  fonts: (fonts, context) => {
