...
export function parseDesignSystemConfig(searchParams: URLSearchParams) {
  // Built from raw (possibly absent) URL params — validated at runtime by
  // designSystemConfigSchema.safeParse below, same as the original's zod
  // schema validating an untyped input object. Kept loosely typed here on
  // purpose (URLSearchParams.get returns string | null for every field);
  // the runtime validator is the source of truth for shape correctness.
  ...
  const result = designSystemConfigSchema.safeParse(
    configInput as unknown as DesignSystemConfigInput,
  )

  if (!result.success) {
    return {
      success: false as const,
      error: result.error.issues[0]?.message ?? "Invalid design system config",
    }
  }
