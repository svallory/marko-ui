...
- [ ] **LICENSE file** — repo declares MIT in `@marko-ui/cli`, `@marko-ui/core`, etc. but there is NO `LICENSE` file at the root. Add one (MIT, holder: Saulo Vallory). Blocks publishing honestly.
...
- [ ] **CLI contract-drift verification pass** (arch-review 2026-08-20 finding 4, sourced from `notes/plans/cli-architecture-review.md` 2026-08-16; NOT re-verified live — verify each before fixing): manifest advertises exit codes 2/4 that no code path returns; `target: "marko"` enforced in some resolution paths but not doctor; two diff renderers with opposite polarity. Also re-judge `src/registry/api.ts:160`'s `npx shadcn build` suggestion — plausibly correct for shadcn-format registry AUTHORS (upstream build tool), unlike the two consumer-facing `npx shadcn init` suggestions in `errors.ts` fixed 2026-08-20.
