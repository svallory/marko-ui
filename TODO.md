...
- [ ] **Chat primitives missing in every style**: `attachment`, `bubble`, `message`, `message-scroller`, `sonner`, `questionnaire`, `direction`, `marker` — no default-registry Marko base exists to restyle; each needs a ground-up port (zag machine or hand-rolled) BEFORE its 8 style variants. Homepage slot for MessageScrollerDemo currently carries CardInstall. `(code — large)`
...
- [ ] nova, vega, lyra, maia, mira, luma, sera — same method as rhea (copy rhea/default part, swap class strings verbatim from data/shadcn-ui/apps/v4/styles/base-<style>/ui). In progress overnight 2026-08-14.
