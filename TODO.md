...
- [ ] **Unblock icon-preview-grid** — the card renders a grid of ~30 icons from the currently selected icon library (shadcn's create page lets you pick Lucide/Hugeicons/etc., and this card previews the set). We have no icon system: our components hand-inline SVG path data per use (`preview-blocks/lib/icon.marko` is a ~65-glyph lookup, not a library). Unblocking needs a real icon strategy decision first: (a) depend on `lucide-static` (or per-library packages) and generate Marko icon tags, or (b) extend the glyph-map approach to full sets (heavy), or (c) only support Lucide and drop the library picker dimension. Once icons are a first-class primitive, the card is a trivial verbatim port. `(design decision, then code — small)`
- [ ] **bar-visualizer (optional)** — NOT blocked, just dead upstream: shadcn ships `preview/cards/bar-visualizer.tsx` but never mounts it in any index.tsx, so parity says leave it out. If we ever want it: it's a hand-rolled canvas + requestAnimationFrame audio-bars card (same class as live-waveform, which we already ported) — direct port, no missing primitives. Only mount it somewhere shadcn doesn't, e.g. our preview-page-3. `(code — small, optional)`
