...
- [ ] **Port bar-visualizer and mount it on preview-page-3** (decided 2026-08-13): dead upstream (shadcn ships `preview/cards/bar-visualizer.tsx` but never mounts it), so it can't live on pages 1/2 without breaking parity — mount it on OUR page (preview-page-3) instead. Hand-rolled canvas + requestAnimationFrame audio-bars card, same class as the already-ported live-waveform: direct port, no missing primitives. `(code — small)`
