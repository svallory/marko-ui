...
- Pass the mechanical parity gate before requesting review:
  ...
  ```
  Demos must be named exactly like upstream's (pairing is by name); stateful demos need `tooling/parity/config/interactions.json` steps (see `tooling/parity/config/INTERACTIONS.md`); accepted differences go in `tooling/parity/config/parity-ignore.json` with a reason. Plus production-build browser verification with real pointer events.
