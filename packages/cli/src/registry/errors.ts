...
export class RegistryInvalidNamespaceError extends RegistryError {
  constructor(public readonly name: string) {
    const message = `Invalid registry namespace: "${name}". Registry names must start with @ (e.g., @marko-ui, @v0).`
...
export class RegistriesIndexParseError extends RegistryError {
  ...
  constructor(parseError: unknown) {
    ...
    super(message, {
      ...
      context: { parseError },
      suggestion:
        "The registries index may be corrupted or have invalid registry namespace format. Registry names must start with @ (e.g., @marko-ui, @example).",
    })
