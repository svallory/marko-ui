...
export async function getProjectInfo(
  ...
  // Marko Run.
  if (allDeps["@marko/run"]) {
    type.framework = FRAMEWORKS["marko-run"]
  ...
  if (
    allDeps["marko"] &&
    (allDeps["@marko/vite"] ||
      configFiles.find((file) => file.startsWith("vite.config."))?.length)
  ) {
    type.framework = FRAMEWORKS["marko-vite"]
    type.frameworkVersion = await getFrameworkVersion(
      type.framework,
      packageJson
    )
...
export async function getFrameworkVersion(
  ...
  const version =
    packageJson.dependencies?.marko || packageJson.devDependencies?.marko
