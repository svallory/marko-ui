...
export function resolvePageTarget(
  ...
  // Marko Run routes live in src/routes/ with +page.marko conventions.
  // Registry page targets use the shadcn `app/<route>/page.<ext>` shape;
  // map them onto the Marko Run filesystem router.
  if (framework === "marko-run") {
    let result = target.replace(/^app\//, "src/routes/")
    result = result.replace(/\/page\.marko$/, "/+page.marko")
...
async function resolveImports(
  ...
  for (const filepath of filePaths) {
    ...
    if (!CODE_EXTENSIONS.includes(path.extname(resolvedPath))) {
      continue
    }
...
export async function rewriteResolvedImportsInContent({
  ...
  // Collect replacements first (the resolver is async), then apply.
  const matches = Array.from(content.matchAll(IMPORT_SPECIFIER_REGEX))
  ...
  for (const match of matches) {
    ...
    if (
      replacements.has(moduleSpecifier) ||
    ...
    replacements.set(moduleSpecifier, newImport)
  }
