 ...
 *
 * base-vega parity: class strings below are copied verbatim (per axis/side)
 * from shadcn styles/base-vega/ui/drawer.tsx's DrawerPrimitive.Popup
...
/**
 * base-vega has no dedicated grabber/handle component class targeted by
 * direction in the same way as the default (vaul-style) port — its
 * equivalent is DrawerSwipeHandle, which base-vega only renders when
