 ...
 *
 * base-nova parity: class strings below are copied verbatim (per axis/side)
 * from shadcn styles/base-nova/ui/drawer.tsx's DrawerPrimitive.Popup
...
/**
 * base-nova has no dedicated grabber/handle component class targeted by
 * direction in the same way as the default (vaul-style) port — its
 * equivalent is DrawerSwipeHandle, which base-nova only renders when
...
// base-nova's DrawerSwipeHandle bakes the round grip indicator into this
// same element's own `after:` pseudo-element rather than a separate child
// (unlike base-rhea, which has no dedicated grabber/handle class and this
// port's precedent of a real `drawer-grabber-indicator` child div — see
// grabberIndicatorBase below). Since restructuring the DOM to drop that
// child is out of scope for a class-string-only pass, the `after:` classes
// are folded into this base string verbatim for byte parity with the tsx
// source; they target a pseudo-element that has no visual effect here
// because nothing renders it (the real indicator is the child div instead).
const grabberBase =
...
const grabberIndicatorBase =
  ...
  " group-data-[swipe-axis=x]/drawer-popup:h-[100px] group-data-[swipe-axis=x]/drawer-popup:w-1.5" +
  " group-data-[swipe-axis=y]/drawer-popup:h-1.5 group-data-[swipe-axis=y]/drawer-popup:w-[100px]";
