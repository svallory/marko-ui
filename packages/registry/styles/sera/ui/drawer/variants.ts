 ...
 *
 * base-rhea parity: class strings below are copied verbatim (per axis/side)
 * from shadcn styles/base-rhea/ui/drawer.tsx's DrawerPrimitive.Popup
...
/**
 * base-rhea has no dedicated grabber/handle component class targeted by
 * direction in the same way as the default (vaul-style) port — its
 * equivalent is DrawerSwipeHandle, which base-rhea only renders when
...
const grabberIndicatorBase =
  "block shrink-0 rounded-none bg-muted" +
