 ...
 *
 * base-lyra parity: class strings below are copied verbatim (per axis/side)
 * from shadcn styles/base-lyra/ui/drawer.tsx's DrawerPrimitive.Popup
...
/**
 * base-lyra has no dedicated grabber/handle component class targeted by
 * direction in the same way as the default (vaul-style) port — its
 * equivalent is DrawerSwipeHandle, which base-lyra only renders when
