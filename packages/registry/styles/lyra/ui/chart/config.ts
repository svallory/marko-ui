...
//
// base-rhea port of ../../../default/ui/chart/config.ts, byte-identical:
// base-rhea's chart.tsx builds `--color-<key>` the exact same way as the
// default (non-rhea) shadcn chart.tsx — same THEMES = { light: "", dark:
// ".dark" } map, same ChartStyle <style> text generation, same ChartConfig
// shape (label/icon + color XOR theme). Byte-parity port of the non-visual
// parts of shadcn's chart.tsx:
