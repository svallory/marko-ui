...
describe("resolveRegistryItemsFromRegistries", () => {
  ...
  it("should resolve non-registry items through @marko-ui registry", () => {
    const items = ["button", "card", "dialog"]
    const config = { registries: {} } as any

    const result = resolveRegistryItemsFromRegistries(items, config)

    // Non-prefixed items are resolved through the built-in @marko-ui registry
    expect(result).toEqual([
      "https://marko-ui.saulo.tech/r/button.json",
      "https://marko-ui.saulo.tech/r/card.json",
      "https://marko-ui.saulo.tech/r/dialog.json",
    ])
    expect(setRegistryHeaders).toHaveBeenCalledWith({})
  })
  ...
  it("should handle mixed registry and non-registry items", () => {
    ...
    // Non-registry items (button, dialog) are resolved through the built-in @marko-ui registry
    expect(result).toEqual([
      "https://marko-ui.saulo.tech/r/button.json",
      "https://v0.dev/chat/b/card/json",
      "https://marko-ui.saulo.tech/r/dialog.json",
