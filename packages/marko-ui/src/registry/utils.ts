...
export async function recursivelyResolveFileImports(
  ...
  // 1. Find all import statements in the file (text-level scan; upstream
  // used ts-morph, but only the module specifiers are needed here).
  const moduleSpecifiers = Array.from(
    content.matchAll(IMPORT_SPECIFIER_REGEX),
    (match) => match[3]
  )
  for (const moduleSpecifier of Array.from(new Set(moduleSpecifiers))) {
