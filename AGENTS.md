...
**Current count as of 2026-09-08 (`apps/docs/scripts/check.sh`, clean cache): 8,378 total.** Of those, **7,164** sit inside the generated `src/tags/verify/**` tree — pure ×9 duplication of the same handful of bugs, one copy per style (verify is generated FROM `src/demos/**`). **1,214** are the real, non-duplicated count across `src/demos`, `src/_blocks`, and `src/routes`. Two upstream-class root causes account for most of the 1,214: a Marko `Input`-component self-collision (TS2749) and an attr-tag `Body<...>` compiler-codegen mismatch (TS2353 + cascading TS7006) — both filed upstream against `@marko/language-tools`/`@marko/type-check`, not fixable from this repo. Use **8,378 total / 1,214 real** as the comparison anchor for the next measurement, not 194 or 15,823.
