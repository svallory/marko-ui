...
**The repo runs on `typescript` ^7 (the native `tsgo` compiler), which ships NO in-process JS compiler API.** `bunx tsc` is tsgo, and `tsc --noEmit -p tooling` works normally — but `import ts from "typescript"` gets you only a version stub: there is no `ts.createProgram`, no `ts.CompilerHost`, no `ts.SyntaxKind`/`ts.SymbolFlags` namespace. Code that used the TS 5 compiler API fails with a wall of TS2339 (`tooling/extract-api.ts` did, 71 of them). The replacement is `typescript/unstable/sync` (plus `typescript/unstable/ast`, `.../ast/is`), whose `API` -> `Project` -> `Checker` exposes the same checker vocabulary. Four traps when writing against it, all learned the hard way in `tooling/extract-api.ts` — read that file before writing new compiler-API tooling:

- **It reads the real file system.** There is no virtual `CompilerHost`, and options load only from a config file, so in-memory sources must be written to disk and a tsconfig generated for them.
- **Declaration paths come back canonicalized** (lower-cased on macOS). Comparing them to a path built with `join()` never matches: in `extract-api.ts` that silently reclassified all 456 `own` and 23 `variant` props as `native` and dropped them from the output, with no error. Normalize both sides.
- **`getDocumentationCommentOfSymbol` is symbol-wide** and concatenates the JSDoc of every declaration. For the comment of one specific declaration, read that node's own `jsDoc` array (whose `.comment` is a node array in TS 7, not a string).
- **Renames:** `questionToken` -> `postfixToken`, `isPropertySignature` -> `isPropertySignatureDeclaration`, and `TypeFormatFlags` folded into `NodeBuilderFlags` (`NoTruncation` and `UseSingleQuotesForStringLiteralType` keep their numeric values). TS 7 also orders string-literal union members alphabetically where TS 5 kept declaration order, so `typeToString` output can reorder without any type changing.

**The sync API does not work under Bun.** Its RPC channel reads Node's `stdout._handle.fd`, which Bun does not expose (`TypeError: undefined is not an object (evaluating 'stdout._handle.fd')`). Tooling that uses it must run under `node`; `tooling/extract-api.ts` re-executes itself under `node --experimental-strip-types` when it detects Bun, so `bun run extract:api` still works.

**`@marko/type-check` is unaffected by the TS 7 move** — it bundles its own typescript (6.0.3), so `marko-type-check` behaviour and error counts are independent of the root `typescript` version.
