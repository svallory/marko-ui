...
- `marko-zag` is `2.0.0-rc.3` (exact) in every package. `bun update` has silently downgraded `packages/shadcn` to `marko-zag@1.2.1` once (2026-08-29): 1.2.1 ships no `<zag>` tag at all, so `<zag>` becomes an unknown tag typed as a native element and every `api().x` call fails with **"Property 'x' does not exist on type 'Element'"** (516 errors). That message means the marko-zag version is wrong, not that `<zag>`'s return type needs generics — verify `grep marko-zag packages/*/package.json apps/docs/package.json` before debugging types. Note that rc.3 moved the tags directory from `tags/` to `src/tags/` and points `marko.json`'s `tags-dir` at it, so a missing `node_modules/marko-zag/tags/zag.marko` is expected on rc.3 and is no longer the tell for a bad version — check the declared version and `marko.json` instead.
