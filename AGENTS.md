...
- `marko-zag` is `2.0.0-rc.2` (exact) in every package. `bun update` has silently downgraded `packages/shadcn` to `marko-zag@1.2.1` once (2026-08-29): 1.2.1 has no `tags/zag.marko`, so `<zag>` becomes an unknown tag typed as a native element and every `api().x` call fails with **"Property 'x' does not exist on type 'Element'"** (516 errors). That message means the marko-zag version is wrong, not that `<zag>`'s return type needs generics — verify `grep marko-zag packages/*/package.json apps/docs/package.json` before debugging types.
