...
## Zero-import tag DX (2026-08-17, decided + implemented)

User decision: steer users to taglib auto-discovery over
`import ... from`; register BOTH PascalCase and dash-case so users pick,
with PascalCase as the collision escape hatch.

Spiked findings (scratchpad/tagspike, all verified by build+serve+mtc):
- Dependency `marko.json` taglibs ARE discovered from package.json deps.
- Explicit entries need the object form: `{"<Badge>": {"template": "..."}}`
  — the string shorthand is parsed as a nested-JSON path and fails.
- `tags-dir` auto-discovery also matches our `badge/badge.marko` layout
  (findAndSetFile tries `index.marko` AND `<dirname>.marko`), but cannot
  express PascalCase names or flattened part names, so we emit explicit
  entries instead.
- PascalCase taglib tags work at runtime and shadow nothing: `<Dialog>`
  (ours) vs `<dialog>` (native) — case-sensitivity solves the native
  collision problem, so NO prefix and no user config is needed. Dash-case
  aliases are emitted only for non-colliding names.
- A local import/variable of the same name still shadows the taglib tag —
  per-file override with zero ceremony.
- Marko's taglib loader REJECTS unknown top-level marko.json keys, so the
  "generated by marko-ui" marker lives in a sibling `.marko-ui-taglib.json`
  stamp file; delete it to take ownership.
- Type-check needs three things for node_modules templates: `script-lang:
  "ts"` in the package taglib, shipped `.d.marko`/`.d.ts` (mtc emit; tooling
  prefers .d.marko over the adjacent .marko, which also stops consumers
  type-checking package internals), and `.ts` extensions rewritten to `.js`
  in emitted declarations (TS5097 otherwise). PascalCase tags additionally
  need ambient globals (`tags.d.ts`) — upstream bug, draft filed in
  notes/upstream-issue-language-tools-pascalcase-taglib.md.

Shipped:
- `build-style-packages.ts` → `@marko-ui/<style>` x9 (default: 86
  components/183 tags). Deps derived from imports + `@types/*` companions.
- CLI `eject` (package → source, taglib regenerated, `#lib/*` subpath
  wired) and `writeProjectTaglib` called from init/add.
- Docs: displayed demo source is rewritten at generation time (component
  imports dropped, alias paths → `@/…`); usage sections lead with the
  no-import tag form and keep the explicit import as the documented
  override; install command is now `bunx marko-ui add <name>`.

Verified round-trip in a scratch app: install `@marko-ui/default` tarball
→ `<Badge>`/`<badge>`/`<Card>`+parts render → `marko-ui eject -y` →
`bun remove @marko-ui/default` → SAME app source builds and renders
identically (md5-compared), mtc clean.

Component bugs found by consumer-side type-checking (fixed at source):
all 8 styles' `sidebar/trigger.marko` imported RHEA's button through the
workspace alias; `input-otp` referenced zag's renamed `PinInputProps`;
`resizable`/`sidebar/provider` Input extends conflicted with native
`onResize`/`content`.
